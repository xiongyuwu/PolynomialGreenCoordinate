	...
	: QGLViewerWidget(parent)
{
	...
	draw_control_point_ok = false;
	draw_move_vertices_ok = false;
	draw_cage_ok = false;
	first_init = true;
	if (!mesh.n_vertices())return;
	PGC.setmesh(mesh);
	PGC.updateControlPoints();
}
...
void MeshViewerWidget::draw_scene_mesh(int drawmode)
{
	...
	if (draw_control_point_ok) {
		PGC.draw_represent_control_points();
	}
	if (draw_cage_ok) {
		PGC.draw_cage();
	}
	if (draw_move_vertices_ok) {
		PGC.draw_position_constraints();
	}
}
...
void MeshViewerWidget::draw_mesh_wireframe()
{
	...
	double g = 150;
	double b = 250;
	glColor4f(r / 255, g / 255, b / 255, 1.0f);
...
{
	//GLfloat light_diffuse[] = { 0.8f, 0.8f, 1.0f, 1.0f }; // 冷色调
	//GLfloat light_position[] = { 1.0f, 1.0f, 1.0f, 0.0f }; // 平行光源
	//glLightfv(GL_LIGHT0, GL_DIFFUSE, light_diffuse);
	//glLightfv(GL_LIGHT0, GL_POSITION, light_position);
	GLfloat global_ambient[] = { 0.6f, 0.6f, 0.8f, 1.0f }; // 全局环境光（淡紫色）
	...
	Mesh::ConstFaceVertexIter fvIt;
	GLfloat mat_a[] = { 174.0 / 255,174.0 / 255, 250.0 / 255, 1.0f }; // 环境光：淡红色
	GLfloat mat_d[] = { 174.0 / 255 * 0.5,174.0 / 255 * 0.5, 250.0 / 255 * 0.5, 1.0f }; // 漫反射：淡红色
	GLfloat mat_s[] = { 0, 0, 0, 1.0f };  // 镜面反射：淡红色
...
void MeshViewerWidget::draw_mesh_solidsmooth() const
{
	...
	GLfloat global_ambient[] = { 0.6f, 0.6f, 0.6f, 1.0f }; // 全局环境光（淡紫色）
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT, global_ambient);
	...
	double g = 150;
	double b = 250;
	GLfloat mat_a[] = { r / 255,g / 255, b / 255, 1.0f }; // 环境光
	GLfloat mat_d[] = { r / 255 * 0.5,g / 255 * 0.5, b / 255 * 0.5, 1.0f }; // 漫反射
	GLfloat mat_s[] = { 0, 0, 0, 1.0f };  // 镜面反射
	GLfloat shine[] = { 0.0f }; // 光泽度
...
void MeshViewerWidget::draw_mesh_pointset() const
{
	...
	glEnd();
	/*glEnableClientState(GL_VERTEX_ARRAY);e
	glVertexPointer(3, GL_DOUBLE, 0, mesh.points());
