...
void InteractiveViewerWidget::mousePressEvent(QMouseEvent *_event)
{
	...
	else
	{
		if (mouse_mode_ != T2_MODE)
		{
			...
			else if (mouse_mode_ == MOVE_CONTROL_POINT) {
				pick_control_point();
			}
			else if (mouse_mode_ == MOVE_POSITION_CONSTRAINTS) {
				pick_position_constraints();
			}
			else if (mouse_mode_ == SELECT_MOVE_CONTROL_POINTS) {
				pick_control_points();
			}
	...
	else
	{
		if (mouse_mode_ != T2_MODE)
		{
			if (mouse_mode_ == MOVE)
			{
				...
				move_control_point_based_selected_control_point(_event->x(), _event->y());
				Mesh::Point P(selectedPoint[0], selectedPoint[1], selectedPoint[2]);
				PGC.set_represent_control_points(selectedControlPoint, P);
				PGC.control_point_is_initialed = true;
				PGC.do_deformation();
				setmesh(PGC.getmesh());
				updateGL();
			}
			else if (mouse_mode_ == MOVE_POSITION_CONSTRAINTS) {
				move_position_constraints_based_selected_position_constraints(_event->x(), _event->y());
				Mesh::Point P(selectedPoint[0], selectedPoint[1], selectedPoint[2]);
				PGC.set_position_constraints(selectedPositionConstraint, P);
				PGC.do_variational_deformation();
				setmesh(PGC.getmesh());
				updateGL();
			}
			else if (mouse_mode_ == MOVE_CONTROL_POINTS) {
				move_selected_control_points(_event->x(), _event->y());
				Mesh::Point P(selectedPoint[0], selectedPoint[1], selectedPoint[2]);
				auto& move_direction = P - mid_point;
				mid_point = P;
				std::vector<Mesh::Point> control_points;

				control_points = PGC.get_represent_control_points();
				for (auto& i : selectedControlPoints) {
					control_points[i] += move_direction;
				}
				PGC.set_represent_control_points(control_points);
				PGC.control_point_is_initialed = true;
				PGC.do_deformation();
				setmesh(PGC.getmesh());
				updateGL();
...
void InteractiveViewerWidget::pick_control_point()
{
	std::vector<Mesh::Point> control_points;
	control_points = PGC.get_represent_control_points();
	double min_distance = DBL_MAX;
	Mesh::Point selected_point(selectedPoint);
	for (int i = 0; i < control_points.size(); i++) {
		double distance = (control_points[i] - selected_point).length();
...
void InteractiveViewerWidget::pick_position_constraints()
{
	std::vector<Mesh::Point> position_constraints;
	position_constraints = PGC.get_position_constraints();

	double min_distance = DBL_MAX;
	Mesh::Point selected_point(selectedPoint);
	for (int i = 0; i < position_constraints.size(); i++) {
		double distance = (position_constraints[i] - selected_point).length();
		if (distance < min_distance) {
			selectedPositionConstraint = i;
			min_distance = distance;
		}
	}
	updateGL();
}
void InteractiveViewerWidget::pick_control_points()
{
	std::vector<Mesh::Point> control_points;
	control_points = PGC.get_represent_control_points();
	double min_distance = DBL_MAX;
	...
	auto& it = std::find(selectedControlPoints.begin(), selectedControlPoints.end(), min_distance_idx);
	if (it == selectedControlPoints.end())
	{
		selectedControlPoints.push_back(min_distance_idx);
		mid_point = (mid_point*(selectedControlPoints.size() - 1) + control_points[min_distance_idx]) /
			selectedControlPoints.size();
	}
	else
	{
		mid_point = (mid_point*selectedControlPoints.size() - control_points[min_distance_idx]) /
			(selectedControlPoints.size() - 1);
		selectedControlPoints.erase(it);
	}
	updateGL();
}
...
void InteractiveViewerWidget::move_control_point_based_selected_control_point(int x, int y)
{
	...
	std::vector<Mesh::Point> control_points;
	control_points = PGC.get_represent_control_points();
	OpenMesh::Vec3d p = control_points[selectedControlPoint];
	gluProject(p[0], p[1], p[2], &ModelViewMatrix[0], &ProjectionMatrix[0], viewport, &winX, &winY, &winZ);

	gluUnProject((GLdouble)(x), (GLdouble)(height() - y), winZ, &ModelViewMatrix[0], &ProjectionMatrix[0], viewport, &selectedPoint[0], &selectedPoint[1], &selectedPoint[2]);
}

void InteractiveViewerWidget::move_position_constraints_based_selected_position_constraints(int x, int y)
{
	if (selectedPositionConstraint == -1)return;
	GLint viewport[4];
	glGetIntegerv(GL_VIEWPORT, viewport);
	GLdouble winX = 0.0;
	GLdouble winY = 0.0;
	...
	OpenMesh::Vec3d p = position_constraints[selectedPositionConstraint];
	gluProject(p[0], p[1], p[2], &ModelViewMatrix[0], &ProjectionMatrix[0], viewport, &winX, &winY, &winZ);

	gluUnProject((GLdouble)(x), (GLdouble)(height() - y), winZ, &ModelViewMatrix[0], &ProjectionMatrix[0], viewport, &selectedPoint[0], &selectedPoint[1], &selectedPoint[2]);
}

void InteractiveViewerWidget::move_selected_control_points(int x, int y)
{
	...
	gluProject(mid_point[0], mid_point[1], mid_point[2], &ModelViewMatrix[0], &ProjectionMatrix[0], viewport, &winX, &winY, &winZ);
...
void InteractiveViewerWidget::draw_interactive_portion(int drawmode)
{
...

	{
		...
		case MOVE_CONTROL_POINT:
			draw_selected_control_point();
		case MOVE_POSITION_CONSTRAINTS:
			draw_selected_position_constraints();
		case SELECT_MOVE_CONTROL_POINTS:
			draw_selected_control_points();
		case MOVE_CONTROL_POINTS:
			draw_selected_control_points();
		default:
			draw_selected_vertex();
			draw_selected_face();
...
void InteractiveViewerWidget::draw_selected_control_point()
{
	if (selectedControlPoint != -1)
	{
		...
		std::vector<Mesh::Point> control_points;
		control_points = PGC.get_represent_control_points();
		glVertex3dv(control_points[selectedControlPoint].data());
		glEnd();
		glPointSize(1);
	}
}

void InteractiveViewerWidget::draw_selected_position_constraints()
{
	if (selectedPositionConstraint != -1)
	{
		Mesh::Point p;
		glColor3f(1.0, 0.5, 0.0);
		glPointSize(12);
		...
		glVertex3dv(position_constraints[selectedPositionConstraint].data());
		glEnd();
		glPointSize(1);
	}
}

void InteractiveViewerWidget::draw_selected_control_points()
{
